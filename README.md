...
6. **Automated Quality Control (Evaluations)**: Внедрена система «LLM-Судья» (`scripts/run_evals.py`). Каждая итерация ответа оценивается по метрикам **Relevance** (соответствие вопросу) и **Faithfulness** (отсутствие галлюцинаций относительно базы знаний).

---

## 🔬 Система оценки качества (Evaluations)

Для поддержания высокой точности ответов используется конвейер оценки:
1. **Трассировка**: Все взаимодействия логгируются в `backend/logs/eval_traces.jsonl` (вопрос, результаты поиска, ответ).
2. **Оценка**: Скрипт `python scripts/run_evals.py` запускает беспристрастного «судью», который выставляет баллы от 1 до 5 за достоверность и релевантность.
3. **Анализ ошибок**: Судья указывает **Root Cause** (первопричину) неудовлетворительных ответов, что позволяет точечно улучшать поисковый индекс или промпты.
